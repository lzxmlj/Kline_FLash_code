...
u8 exfuns_init(void)
{
	...
	for(i=0;i<3;i++)
	{
		fs[i]=(FATFS*)mymalloc(0,sizeof(FATFS));	
		if(!fs[i])break;
	}
	file=(FIL*)mymalloc(0,sizeof(FIL));		
	ftemp=(FIL*)mymalloc(0,sizeof(FIL));		
	fatbuf=(u8*)mymalloc(0,512);				
	if(i==3&&file&&ftemp&&fatbuf)return 0;  
	else return 1;	
}
