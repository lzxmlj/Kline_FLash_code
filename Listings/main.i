...
CanTxMsg TxMsg2={0x700,0,((uint32_t)0x00000000),((uint32_t)0x00000000),8,{0xAC,99,80,50,40,0,0,0}};
CanTxMsg TxMsg1={0x600,0,((uint32_t)0x00000000),((uint32_t)0x00000000),8,{0xAC,99,80,50,40,0,0,0}};
u32 delay_time;
u8 start_load_data;
int test_temp;
u8 check_sum;
...
extern u8 CAN_need_update, test_buff[8], CAN_need_update_data[8];
...
int main(void)
 {
  ...
  FSMC_SRAM_Init();
   ...
   GPIO_WriteBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0000)),((uint16_t)0x0200),Bit_SET);
   while(1)
	{
	  if(loop_enable == 1)
	  {
	   ...
	   if((can_receive_data == 0x800) && (check_over == 0) && (start_load_data == 1))
		 {
		  for(i = 0;i < can_receive_data;i++)
		  {
			if(VcRAM_Utilty[i] != data_list[i])
			  check_fail = 1;
		  }
				  test_message[0] = 0x55;
				  test_message[1] = 0xAA;		  
		  ...
		  start_load_data = 0;
			  ...
			  else if((can_receive_data == 0x6000) && (check_over == 1))
			{
				check_fail = 0;
		  for(i = 0;i < can_receive_data;i++)
		  {
			if(VcRAM_Cal[i] != flash_data[i])
			  check_fail = 1;
		  }
				  test_message[0] = 0xAA;
				  test_message[1] = 0x055;			  
		...
		else if((can_receive_data == 0x35000) && (check_over == 2))
			{
		  for(i = 0;i < can_receive_data;i++)
		  {
			if(VcRAM_Rom[i] != flash_rom[i])
			  check_fail = 1;
		  }
				  test_message[0] = 0x01;
				  test_message[1] = 0x02;		  
			...
			&& (CAN_need_update_data[6] == 3) && (CAN_need_update_data[7] == 3))
	   {
		...
		if(( count != 70) && ( initial_complete == 0))
		{
			...
			if((count == 20) && ( initial_complete == 0))
			{
				...
				GPIO_WriteBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0000)),((uint16_t)0x0200),Bit_RESET);
			}
		...
		else if((initial_complete == 5)&&(Vbmessage_send == 0)&&(count >= 1))
		{
			check_sum = 0;
			if(flash_block <= 15)
			{
		...
		else if((initial_complete == 7)&&(Vbmessage_send == 0)&&(count >= 10000))
		{
		...
		else if((initial_complete == 9)&&(Vbmessage_send == 0)&&(count >= 1))
		{
			check_sum = 0;
			if(cal_is_over == 0)
			{
	  ...
	  void Usart_Send(u8 ch)
	  {
		 while((((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1000))->SR&0X40)==0);
		  ((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1000))->DR= ch;
	  }
void RTE_INITIAL(void)
{
	...
	TIM_ClearFlag(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0000)), ((uint16_t)0x0001));
...
void TIM3_INITIAL(void)
{
	...
	TIM_ClearFlag(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0400)), ((uint16_t)0x0001));
	TIM_ITConfig(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0400)),((uint16_t)0x0002),ENABLE);
